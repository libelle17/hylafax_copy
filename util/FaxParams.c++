...
const int FaxParams::BITNUM_T37		=  1;
const int FaxParams::BITNUM_T38		=  3;
...
const int FaxParams::FEATURE_SSLFAX	= 1;
...
void FaxParams::setFeature(int bitNum, bool val)
{
    if (val) {
	features |= (1<<(bitNum-1));
    } else {
...
bool FaxParams::getFeature(int bitNum)
{
    return (features & (1<<(bitNum-1)));
}
