...
u_char* MemoryDecoder::encodeTagLine(u_long* raster, u_int th, u_int slop, uint32 scanlines, bool addLines, bool onBottom)
{
    /*
    ...
    if (!onBottom && !addLines) decode(NULL, width, th);		// discard decoded data
    if (!isG4) {
	/*
	...
	if (!onBottom) enc.encode(raster, width, th, (unsigned char*) refrow);
	/*
	...
	if (!RTCraised()) {
		...
		lines++;
		if (scanlines && !addLines && onBottom && lines >= (scanlines-th)) break;
	    }
	}
	if (onBottom) enc.encode(raster, width, th, (unsigned char*) refrow);
	delete raster;
