...
Class1Modem::sendPrologue(FaxParams& dcs_caps, const fxStr& tsi)
{
	...
	return (false);
#if defined(HAVE_SSL)
    if (!isSSLFax && conf.class1SSLFaxSupport && conf.class1SSLFaxInfo.length() && \
...
Class1Modem::sendTraining(Class2Params& params, int tries, fxStr& emsg)
{
	...
	do {
	     ...
	     */
	    HDLCFrame frame(conf.class1FrameOverhead);
	    bool gf = recvFrame(frame, FCF_SNDR, conf.t4Timer);
#if defined(HAVE_SSL)
		...
		if (!getSSLFaxConnection()) {
		    ...
		    sslfax.acceptClient(sslFaxProcess, sslFaxPasscode, conf.class1SSLFaxClientTimeout);
		    ...
		    if (!isSSLFax) {
			gf = (waitFor(AT_CONNECT, conf.t4Timer) && recvFrame(frame, FCF_SNDR, conf.t4Timer, true));
		    } else {
			gf = recvFrame(frame, FCF_SNDR, conf.t4Timer);
		    }
		}
	    }
#endif
	...
	do {
		...
		switch (pprframe.getFCF()) {
			...
			if (doctceor) {
			    ...
			    if (!isSSLFax && !useV34 && curcap->mod == V17 && badframes == frameNumber && FaxModem::getPageNumberOfCall() == 1) {
